...
function bucketTitle(log: MockLog, now: Date): string {
...
export function useSectionedLogs(logs: MockLog[], now: Date): LogSection[] {
  return useMemo(() => {
    const bucketMap = new Map<string, MockLog[]>();
